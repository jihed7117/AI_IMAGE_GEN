...
/// Converts an IEEE 754 half-precision (float16) bit pattern to a double.
double _halfToDouble(int bits) {
  ...
  if (exp == 0) {
    value = frac == 0
        ? 0.0
        : (frac / 1024.0) * math.pow(2, -14).toDouble();
  } else if (exp == 0x1F) {
    value = frac == 0 ? double.infinity : double.nan;
  } else {
    value = (1.0 + frac / 1024.0) * math.pow(2, exp - 15).toDouble();
  }
  return sign == 1 ? -value : value;
}

/// Converts a double to an IEEE 754 half-precision (float16) bit pattern,
/// rounding to nearest even.
int _doubleToHalf(double value) {
  if (value.isNaN) return 0x7E00;
  if (value.isInfinite) return value > 0 ? 0x7C00 : 0xFC00;
  if (value == 0.0) return value.isNegative ? 0x8000 : 0x0000;
  final f = Float32List(1)..[0] = value;
  final bits = f.buffer.asByteData().getUint32(0, Endian.little);
  final sign = (bits >> 16) & 0x8000;
  final exp = ((bits >> 23) & 0xFF) - 127 + 15;
  ...
  if (exp <= 0) {
    ...
    var half = (mant | 0x800000) >> (1 - exp + 13);
    final rem = (mant | 0x800000) & ((1 << (1 - exp + 13)) - 1);
    final halfway = 1 << (1 - exp + 12);
...
abstract class OrtValue {
  ...
  List<num> _getNumList(ffi.Pointer<bg.OrtValue> ortValuePtr) {
    ...
    if (tensorElementType == ONNXTensorElementDataType.uint8) {
      ...
      calloc.free(dataPtrPtr);
    } else if (tensorElementType == ONNXTensorElementDataType.float16) {
      // fp16 models (e.g. stable-diffusion-1.5-onnx-fp16) return half-precision
      // tensors; read them as uint16 and convert to doubles.
      final dataPtrPtr = calloc<ffi.Pointer<ffi.Uint16>>();
      final dataPtr = _getTensorMutableData(ortValuePtr, dataPtrPtr);
      for (int i = 0; i < tensorShapeElementCount; ++i) {
        data.add(_halfToDouble(dataPtr[i]));
      }
      calloc.free(dataPtrPtr);
...
class OrtValueTensor extends OrtValue {
  ...
  static OrtValueTensor createTensorWithDataList(List data,
    ...
    if (element is Uint8List) {
      ...
      dataPtr = (calloc<ffi.Int64>(dataSize)
            ..asTypedList(dataSize).setRange(0, dataSize, flattenData))
          .cast();
      dataByteCount = dataSize * 8;    } else if (element is Float32List) {
  ...
  /// Creates a float16 (half-precision) tensor from [values]. fp16 models
  /// (e.g. stable-diffusion-1.5-onnx-fp16) declare float16 inputs, which the
  /// regular typed-list factories cannot produce.
  static OrtValueTensor createTensorWithFloat16List(
      List<double> values, List<int> shape) {
    final bits = Uint16List(values.length);
    for (var i = 0; i < values.length; i++) {
      bits[i] = _doubleToHalf(values[i]);
    }
    final dataSize = bits.length;
    final ffi.Pointer<ffi.Void> dataPtr =
        (calloc<ffi.Uint16>(dataSize)
              ..asTypedList(dataSize).setRange(0, dataSize, bits))
            .cast();
    return _createTensor(
        ONNXTensorElementDataType.float16.value, dataPtr, dataSize * 2, shape);
  }

  static OrtValueTensor _createTensor(
      int dataType, ffi.Pointer<ffi.Void> dataPtr, int dataByteCount,
      List<int> shape) {
    final shapeSize = shape.length;
    final shapePtr = calloc<ffi.Int64>(shapeSize);
    shapePtr.asTypedList(shapeSize).setRange(0, shapeSize, shape);
